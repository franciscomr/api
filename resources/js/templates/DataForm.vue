...
<template>
  <Alert v-if="showNotification" @alert_displayed="alert_displayed" :message="'Se ha creado el siguiente Recurso:'"
    :resource1="resourceCreated.value" />
  <div :class="full_display ? '' : 'w-full sm:max-w-md px-5 mx-auto bg-white py-5 rounded-xl'">
    <div v-if="!full_display" class="flex justify-between pb-4">
      <h1 class="font-medium">{{ title }}</h1>
      <span @click="$router.go(-1)" class="cursor-pointer"> <close-icon :size=24 fillColor="#64748b" /></span>
    </div>
